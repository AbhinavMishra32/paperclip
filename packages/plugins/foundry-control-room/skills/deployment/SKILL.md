...
5. A successful push or accepted deployment is not a successful release. Poll `foundry.control-room:get_vercel_deployments` until that deployment reaches `READY` or `ERROR`. On `ERROR`, call `foundry.control-room:get_vercel_deployment_events` with its deployment ID and fix the concrete build error before deploying again. Do not call `deploy_project` again while the accepted deployment is still `BUILDING` or `QUEUED`.
